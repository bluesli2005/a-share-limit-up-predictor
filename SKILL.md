...
For the scheduled 14:40 collection, use `scripts/collect_with_retries.py`. Treat a missing file, non-success status, zero records, stale or non-provider-native timestamp, truncated candidate scan, or missing mandatory candidate fields from either the market snapshot or limit-up pool as a failed attempt. Retry the complete pair immediately, up to three total attempts with a short bounded delay. Never combine market data from one attempt with a pool from another; select and record one paired `selected_attempt`. Preserve every attempt and stop after the third failure; never loop indefinitely. The final report must state the attempt count, distinguish transport success from formal data usability, and still produce a data-quality failure summary when all attempts fail.
...
When a thread-attached scheduled automation is configured, display the completed 14:40 result in that thread after the local run finishes. If formal scoring fails, display the data-quality failure and mechanically observable candidates instead of staying silent or inventing probabilities.
