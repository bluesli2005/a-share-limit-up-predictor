...
For the scheduled 14:40 collection, use `scripts/collect_with_retries.py`. Treat a missing file, non-success status, or zero records from either the market snapshot or limit-up pool as a failed attempt. Retry the complete pair immediately, up to three total attempts with a short bounded delay. Preserve every attempt and stop after the third failure; never loop indefinitely. The final report must state the attempt count and still produce a data-quality failure summary when all attempts fail.
