...
=======
11:30和14:40均由`scripts/collect_with_retries.py`编排采集。重试只处理“没有取得成对数据”的情况：行情或涨停池文件缺失、JSON无效、状态失败、记录数为0或记录列表为空时，整组重试，最多3次。行情与涨停池必须来自同一次尝试。只要同一次尝试的两侧都成功取得非空数据，立即停止采集，不因缺少原生时间戳、实际涨停价或其他正式评分字段而重复请求。这些缺口继续记录为质量问题，并让最终报告降级为`data_quality_failure`。每次证据保存在对应`raw/.../attempts/`目录，尝试摘要保存为`collection-attempts.json`，记录`collection_succeeded`、`formal_data_usable`、停止原因、质量问题和最终选中的成对尝试`selected_attempt`。连续3次仍未取得成对数据时停止，不继续循环。
>>>>>>> Stashed changes
...
Markdown是正式阅读报告，并由线程自动化在每个交易日展示到Chat；JSON只供逐日比较和回测，默认不在Chat展示或提供链接，除非用户明确要求查看。JSON必须包含`schema_version`、`trade_date`、`generated_at_cst`、`run_type`、数据源和数据时间戳；预测记录还应包含`target_trade_date`和模型输出字段。数据不足时仍保存失败报告，但不得生成伪概率。不得覆盖其他运行时点的文件。
